-- Funded accounts exist, and backtesting is no longer something I do here.
--
-- 'Funded' is added to both tables; 'Backtest (FX Replay)' stays LEGAL and
-- stops being OFFERED. Removing it from the CHECK would make every backtest
-- trade already on record unwritable — the app quietly destroying its own
-- history to tidy a dropdown. Old entries keep their label, still appear in the
-- account switcher, and can still be edited; the capture form just will not let
-- a new one be filed under it.
--
-- The default moves to 'Live' for the same reason it was 'Backtest' before: a
-- default should be the thing you actually do most, and a wrong one silently
-- files real trades under the wrong account.
...
CREATE TABLE trades_rebuilt (
  ...
  account       TEXT NOT NULL DEFAULT 'Live'
                CHECK (account IN ('Backtest (FX Replay)','Demo','Live','Funded')),
  account_label TEXT,
...
-- The same widening for money movements.
CREATE TABLE cash_events_rebuilt (
  id         TEXT PRIMARY KEY,
  account    TEXT NOT NULL CHECK (account IN ('Backtest (FX Replay)','Demo','Live','Funded')),
...
INSERT INTO cash_events_rebuilt SELECT id, account, kind, amount, date, note, created_at FROM cash_events;
DROP TABLE cash_events;
ALTER TABLE cash_events_rebuilt RENAME TO cash_events;
